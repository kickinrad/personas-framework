...
def selected_codex_mcps(servers: dict[str, object], names: list[str]) -> dict[str, dict[str, object]]:
    """Validate and project only explicitly selected private bindings into Codex."""
...
def main() -> int:
    ...
    try:
        persona = args.persona.resolve(); slug, name, description, digest = persona_data(persona)
        servers, declared_names = load_mcp_payload(persona)
        codex_mcps = selected_codex_mcps(servers, [*declared_names, *args.codex_mcp])
